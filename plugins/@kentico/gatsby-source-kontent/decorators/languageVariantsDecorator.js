...
const decorateItemsWithLanguageVariants = (defaultCultureContentItemNodes, nonDefaultLanguageItemNodes) => {
  for (const [languageCodename, currentLanguageNodes] of Object.entries(nonDefaultLanguageItemNodes)) {
    ...
    for (const [otherLanguageCodename, otherLanguageNodes] of Object.entries(nonDefaultLanguageItemNodes)) {
...
const decorateItemNodeWithLanguageVariantLink = (itemNode, allNodesOfAnotherLanguage) => {
  ...
  const languageVariantNode = allNodesOfAnotherLanguage.find(nodeOfSpecificLanguage => itemNode.system.codename === nodeOfSpecificLanguage.system.codename && itemNode.system.type === nodeOfSpecificLanguage.system.type);
...
const makeLink = (firstItem, secondItem) => {
  const existingLink = firstItem && secondItem && secondItem.otherLanguages___NODE && secondItem.otherLanguages___NODE.find(otherLanguageId => otherLanguageId === firstItem.id);
