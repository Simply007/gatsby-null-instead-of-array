  ...
  Object.keys(itemNode.elements).forEach(propertyName => {
    ...
    if (property.type === 'modular_content') {
      ...
      const linkedNodes = allNodesOfSameLanguage.filter(node => {
        const match = property.itemCodenames.find(codename => codename && codename === node.system.codename);
        return match !== undefined && match !== null;
      });
      normalize.addLinkedItemsLinks(itemNode, linkedNodes, linkPropertyPath, property.itemCodenames);
    }
  });
};
