...
const get = async (client, createNodeId, includeRawContent) => {
  const contentTypesResponse = await client.types().toPromise();
...
const createContentTypeNode = (createNodeId, contentType, includeRawContent) => {
  ...
  return normalize.createKcArtifactNode(nodeId, contentType, `type`, contentType.system.codename, additionalData, includeRawContent);
};
