...
exports.sourceNodes = async ({
 ...
 ${_.get(deliveryClientConfig, 'projectId')}`);
  ...
  if (enableLogging) {
    console.info(`Creating type nodes schema.`);
  }

  await typeNodesSchema.createTypeNodesSchema(client, schema, createTypes);
  const defaultCultureContentItemNodes = await itemNodes.getFromDefaultLanguage(client, defaultLanguageCodename, contentTypeNodes, createNodeId, includeRawContent);
  const nonDefaultLanguageItemNodes = await itemNodes.getFromNonDefaultLanguage(client, nonDefaultLanguageCodenames, contentTypeNodes, createNodeId, includeRawContent);
  languageVariantsDecorator.decorateItemsWithLanguageVariants(defaultCultureContentItemNodes, nonDefaultLanguageItemNodes);
  const allItemNodes = defaultCultureContentItemNodes.concat(_.flatten(Object.values(nonDefaultLanguageItemNodes)));
...
const addHeader = (deliveryClientConfig, trackingHeader) => {
  ...
  let headers = _.cloneDeep(deliveryClientConfig.globalQueryConfig.customHeaders);
  ...
  if (headers.some(header => header.header === trackingHeader.header)) {
    ...
    headers = headers.filter(header => header.header !== trackingHeader.header);
  }
