...
const urlSlugElementDecorator = require('./decorators/urlSlugElementDecorator');
/**
...
const getFromDefaultLanguage = async (client, defaultLanguageCodename, contentTypeNodes, createNodeId, includeRawContent = false) => {
  const contentItemsResponse = await client.items().languageParameter(defaultLanguageCodename).toPromise();

  const allItems = _.unionBy(contentItemsResponse.items, Object.values(contentItemsResponse.linkedItems), 'system.codename');

  richTextElementDecorator.resolveData(allItems);
  urlSlugElementDecorator.resolveUrls(allItems);
  ...
  const contentItemNodes = itemsFlatted.map(contentItem => {
    try {
      ...
      return createContentItemNode(createNodeId, contentItem, contentTypeNodes, includeRawContent);
    } catch (error) {
      console.error(error);
    }
  });
...
const getFromNonDefaultLanguage = async (client, nonDefaultLanguageCodenames, contentTypeNodes, createNodeId, includeRawContent = false) => {
  const nonDefaultLanguageItemNodes = {};

  for (const languageCodename of nonDefaultLanguageCodenames) {
    const languageResponse = await client.items().languageParameter(languageCodename).toPromise();

    const allItems = _.unionBy(languageResponse.items, Object.values(languageResponse.linkedItems), 'system.codename');
    ...
    const contentItemsNodes = languageItemsFlatted.map(languageItem => {
      ...
      return createContentItemNode(createNodeId, languageItem, contentTypeNodes, includeRawContent);
    });
...
const createContentItemNode = (createNodeId, contentItem, contentTypeNodes, includeRawContent = false) => {
  ...
  const languageParamCase = changeCase.paramCase(contentItem.preferred_language);
  const nodeId = createNodeId(`kentico-kontent-item-${codenameParamCase}-${languageParamCase}`);
  ...
  return normalize.createKcArtifactNode(nodeId, itemWithElements, `item`, contentItem.system.type, additionalData, includeRawContent);
};
